...
삭제에서는 데이터 검증이 중요하지는 않습니다. 하지만 미들웨어는 호출해야 할 수도 있습니다. 예를 들면, 어떤 오브젝트가 삭제될 때 함께 정리되어야 하는 의존성이 있는 경우가 있을 수 있겠죠. 따라서 미들웨어 호출을 보장하기 위해선 아래와 같이 먼저 도큐먼트를 불러온 뒤 삭제해야 합니다.
